...
BEGIN
    ...
    -- The dbid lookup is inlined here on purpose: as a standalone statement it
    -- would be tracked separately by pg_stat_statements under a text that does
    -- not mention awr_lite, and would leak into the report's own Top SQL.
    --
    -- The repository's own statements are excluded HERE, at capture time,
    -- against the full untruncated query text. Filtering only at report time
    -- is not enough: with pg_stat_statements.track = all, the report's
    -- internal section queries are tracked too, and their long SELECT lists
    -- push the identifying "awr_lite" schema reference beyond the 1000
    -- characters of text the repository stores.
    EXECUTE format($q$
...
RETURNS TABLE (queryid bigint, username text, calls bigint, total_exec_time_ms numeric,
    ...
    SELECT e.queryid, r.rolname,
     ...
     WHERE e.snap_id = p_end
       ...
       AND e.query NOT LIKE '%cron.%'      -- hide pg_cron's own bookkeeping
...
RETURNS TABLE (schemaname text, relname text, phys_blk_reads bigint, blk_hits bigint,
    ...
    SELECT e.schemaname, e.relname,
     ...
     WHERE e.snap_id = p_end
       AND (p_include_repo OR e.schemaname NOT IN ('awr_lite', 'cron'))
...
-- Unused-index candidates, scoped the way an Oracle AWR reader would expect:
--   * only indexes whose TABLE was active in the interval (an idle table's
--     indexes are trivially unscanned and would flood the report - a database
--     with thousands of indexes must not print thousands of lines here)
--   * excludes primary key and unique indexes, which enforce constraints and
--     are not drop candidates regardless of scan count
--   * excludes the awr_lite repository and pg_cron's own schema
--   * largest first (wasted-space order), capped at p_limit rows
CREATE OR REPLACE FUNCTION awr_lite.report_unused_indexes(
    p_begin bigint, p_end bigint, p_limit int DEFAULT 15,
    p_include_repo boolean DEFAULT false)
RETURNS TABLE (schemaname text, relname text, indexrelname text,
               index_size text, table_activity_in_interval bigint) AS $fn$
    WITH active_tables AS (
        SELECT e.schemaname, e.relname,
               ((COALESCE(e.seq_scan,0) - COALESCE(b.seq_scan,0)) +
                ...
                (e.n_tup_ins - COALESCE(b.n_tup_ins,0)) +
                (e.n_tup_upd - COALESCE(b.n_tup_upd,0)) +
                (e.n_tup_del - COALESCE(b.n_tup_del,0))) AS activity
          FROM awr_lite.stat_tables e
          LEFT JOIN awr_lite.stat_tables b
            ON b.snap_id = p_begin AND b.relid = e.relid
         WHERE e.snap_id = p_end)
    SELECT e.schemaname, e.relname, e.indexrelname,
      ...
      JOIN active_tables t
        ON t.schemaname = e.schemaname AND t.relname = e.relname
      JOIN pg_index i                      -- also drops indexes deleted since
        ON i.indexrelid = e.indexrelid     -- the snapshot: nothing to act on
     WHERE e.snap_id = p_end
       AND (p_include_repo OR e.schemaname NOT IN ('awr_lite', 'cron'))
       AND (COALESCE(e.idx_scan,0) - COALESCE(b.idx_scan,0)) = 0
       AND t.activity > 0                  -- table was used; index was not
...
BEGIN
    ...
    IF v_all OR sec = 'INDEXES' THEN
        RETURN NEXT repeat('-', W);
        RETURN NEXT '  UNUSED INDEXES ON ACTIVE TABLES (largest first)';
        RETURN NEXT repeat('-', W);
        RETURN NEXT format('  %-18s %-26s %-32s %12s', 'Schema','Table','Index','Size');
        n := 0;
        FOR r IN SELECT * FROM awr_lite.report_unused_indexes(p_begin, p_end, p_limit) LOOP
            n := n + 1;
            RETURN NEXT format('  %-18s %-26s %-32s %12s',
                left(r.schemaname,18), left(r.relname,26),
                left(r.indexrelname,32), r.index_size);
        END LOOP;
        IF n = 0 THEN
            RETURN NEXT '  (No unused non-constraint indexes on tables that were active in this interval.)';
        ELSIF n = p_limit THEN
            RETURN NEXT format('  (Showing the %s largest. Call awr_lite.report_unused_indexes(%s, %s, <n>) for more.)',
                               p_limit, p_begin, p_end);
        END IF;
        RETURN NEXT '  (Only tables with activity in the interval are considered; primary key and';
        RETURN NEXT '   unique indexes are excluded. Evaluate over a long window before dropping.)';
        RETURN NEXT '';
    END IF;
    ...
    IF v_all OR sec = 'ASH' THEN
        ...
        END LOOP;
        IF n = 0 THEN
            RETURN NEXT '  (No active sessions were captured between these snapshots. The sampler only';
            RETURN NEXT '   records sessions that are active at the instant it runs, so statements';
            RETURN NEXT '   shorter than the sampling interval can fall between samples. If the sampler';
            RETURN NEXT '   is not yet scheduled, schedule awr_lite.sample_activity() with pg_cron.)';
        END IF;
    ...
    IF v_all OR sec = 'SOURCES' THEN
        ...
        END LOOP;
        IF n = 0 THEN
            RETURN NEXT '  (No active sessions were captured between these snapshots, so there is';
            RETURN NEXT '   nothing to attribute. See the wait event section note above. This section';
            RETURN NEXT '   additionally requires PostgreSQL 14 or higher for the query ID.)';
        END IF;
